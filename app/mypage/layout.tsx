...
export default function MyPageLayout({side, menu}: {side: React.ReactNode; menu: React.ReactNode}) {
  ...
  return (
    <>
      {/* pc, 태블릿 */}
      <div className="hidden min-h-[60rem] bg-black-400 tablet:block dark:bg-slate-800">
      ...
      {/* 모바일 */}
      <div className="min-h-[60rem] bg-black-400 tablet:hidden dark:bg-slate-800">
        <div className="mx-auto px-4 pt-6 tablet:p-6 pc:w-full pc:max-w-[75rem] pc:p-0 pc:pt-[4.5rem]">
          <aside>{side}</aside>
          {isModalOpen && (
            <OverlayContainer>
              <div className="h-full w-full overflow-y-auto bg-white dark:bg-slate-800">
                <div className="min-h-[60rem] bg-black-400 px-4 pt-6 dark:bg-slate-800">{menu}</div>
              </div>
            </OverlayContainer>
          )}
        </div>
