...
export default function SortSelect({selectedSort, onSelectedSort}: CustomSelectProps) {
  ...
  return (
    <div
      ...
      className="relative h-41pxr w-90pxr min-w-[90px] cursor-pointer rounded-2xl border border-nomad-black bg-white px-2 py-2 dark:border-slate-600 dark:bg-slate-900/10 tablet:h-53pxr tablet:w-120pxr tablet:px-5 tablet:py-4 pc:w-127pxr"
    >
      ...
      {isOpen && (
        <ul className="no-scrollbar absolute right-1pxr z-10 mt-4 w-[90px] rounded-md border border-gray-200 bg-none shadow-sidenavi-box dark:border-slate-600 tablet:w-[120px] pc:w-127pxr">
          {options.map(option => (
            <li
              key={option.value}
              className="no-scrollbar w-[90px] border-collapse cursor-pointer border-t border-gray-200 bg-white px-5pxr py-18pxr text-md font-medium text-gray-800 hover:bg-green-50 hover:text-nomad-black dark:border-slate-500 dark:bg-slate-400 tablet:w-[120px] tablet:text-2lg pc:w-127pxr"
